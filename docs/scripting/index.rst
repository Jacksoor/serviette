...
When a user executes a script (either via ``!command arg`` or ``$escrow amount command arg``), a script is ``exec``\ed in the sandbox and the argument sent to its stdin. Its stdout and stderr, up to a limit (generally 10MB each) are captured and sent to the entity requesting execution, as well as exit code.
