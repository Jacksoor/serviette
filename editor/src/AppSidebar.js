      ...
      <div className="bg-faded sidebar">
        ...
        <Nav vertical pills>
          {this.props.account.scripts.map((script) => {
            ...
            return <NavItem key={script.name}><NavLink href="#" className={script.published ? null : "text-muted"} active={current} onClick={this.props.onScriptSelected.bind(this, script)}><samp>{script.name}</samp>{this.props.unsaved && current ? <sup>*</sup> : ''}</NavLink></NavItem>;
          })}
        </Nav>
      </div>
      );
